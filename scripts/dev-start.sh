...
echo "=== Zor Local Dev Environment ==="
echo ""
...
if [ ! -f .env ]; then
  ...
  sed -i 's|NEXTAUTH_URL="http://localhost:3000"|NEXTAUTH_URL="https://zor.local"|' .env
  echo "  Generated secrets and set NEXTAUTH_URL=https://zor.local"
fi

# ── 2. Add zor.local to /etc/hosts if needed ──
LOCAL_IP=$(hostname -I | awk '{print $1}')
if ! grep -q "zor.local" /etc/hosts 2>/dev/null; then
  echo "Adding zor.local to /etc/hosts (requires sudo)..."
  echo "${LOCAL_IP} zor.local" | sudo tee -a /etc/hosts > /dev/null
fi
...
pnpm --filter @zor/db db:push 2>&1 | tail -3

echo "Seeding exercise library..."
pnpm --filter @zor/db db:seed 2>&1 | tail -3

echo "Seeding dev test data..."
pnpm --filter @zor/db db:seed:dev 2>&1 | tail -10
...
echo ""
echo "============================================"
echo "  Zor dev environment is ready!"
echo ""
echo "  URL:  https://zor.local"
