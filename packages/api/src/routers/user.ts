...
export const userRouter = createTRPCRouter({
  ...
  registerPushToken: rateLimitedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      // Security: the token string is an opaque id generated by Expo/FCM/APNs
      // for a specific device. If the caller presents a token that's already
      // registered to a different user, we must NOT silently rewrite its
      // userId — that would let anyone who got hold of a token (logs, MITM,
      // a stolen backup) redirect another user's push notifications to
      // themselves. Only the original owner can re-register.
      const existing = await ctx.db.pushToken.findUnique({
      ...
      if (existing && existing.userId !== ctx.user.id) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Push token is registered to a different account",
        });
      }
