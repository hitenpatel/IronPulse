...
Zor is sharp, fast, premium, and training-focused. Its mark follows the approved JPEG reference: a compact 1.14:1 geometric `Z` made from two interlocking filled components separated by a narrow parallel negative-space slash. The second component is an exact 180-degree rotation of the first, including rail length, taper, sharp diagonal tip, and shallow outer-terminal curve; no intermediate vertices or kinks are permitted. The paired components sit 1.5 units closer together than the raw threshold trace while preserving the outer silhouette. Both sharp diagonal tips extend to the same outer bounds as the opposite top and bottom rails. The canonical coordinates are traced from the clean standalone mark in that reference and must not be replaced by an invented three-rail construction. It must remain recognizable at launcher-icon and favicon sizes without depending on texture, shadows, gradients, or a font.
