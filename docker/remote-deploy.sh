...
IMAGE_TAG="$TAG" $COMPOSE pull ironpulse
...
if [ "$ENV" = "staging" ]; then
  IMAGE_TAG="$TAG" $COMPOSE run --rm --entrypoint sh ironpulse -c "cd /app/packages/db && $WAIT_PG && prisma migrate deploy && prisma db seed && tsx seeds/seed-dev.ts"
else
  IMAGE_TAG="$TAG" $COMPOSE run --rm --entrypoint sh ironpulse -c "cd /app/packages/db && $WAIT_PG && prisma migrate deploy && prisma db seed"
fi
